...
using Statistics, Random, Distributions, Combinatorics
using Flux, Tracker, Printf
...
function write_res2(df_test, mod, file, var, combn, rep)
    df_tmp = select(df_test, Not(:origin))
    io = open(file, "a")
    for i in 1:size(df_tmp, 1)
        new = [df_tmp[i, k] for k in 1:size(df_tmp, 2)]
        res = Tracker.data(mod(new))
        for j in 1:size(res, 1)
            write_res_unit2(var, combn, rep, i, j, df_test.origin[i], res[j], io)
        end
    end
    close(io)
    return nothing
...
function write_res_unit2(var, combn, rep, id_sampl, reg_hyp, reg_true, val, io)
    println(io, var, " ", combn, " ", rep, " ", id_sampl, " ", reg_hyp, " ", reg_true, " ", val)
    return nothing
end
...
function simu_train(df_train, sig = 0.0, nrep::Int64 = 10, nreg::Int64 = 3)
    df_tmp = select(df_train, Not(:origin))
    train_data = []
    # size of training set if nrep*size(df, 1)
    for r in 1:nrep
...
# a second version to write all simulatoin in a single file
function simu_ml2(df_train, df_test, sig, file::String, var, combn, rep,
    aug = 1e5, nreg::Int64 = 3)
    ##
    ndim = size(df_train, 2) - 1
    nhid = round(Int, (ndim + nreg) / 2)
    ##
    ...
    # println("✓")
    ##
    write_res2(df_test, m, file, var, combn, rep)
    ##
    return nothing
end
